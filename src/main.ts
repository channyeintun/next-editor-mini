...
editorActor.subscribe((snapshot) => {
  const { text, selectionStart, selectionEnd, selectionDirection } = snapshot.context;
  if (editorEl.value !== text) {
    editorEl.value = text;
  }
  if (player.getSnapshot().value === "playing") {
    editorEl.focus();
    editorEl.setSelectionRange(selectionStart, selectionEnd, selectionDirection);
  }
